...
def texturing(best_particle, RGB, MAP, depth_dt, tm):
    ...
    ind_sw = np.where(np.logical_and(sw[2, :, :] <= 1, sw[2, :, :] >= -0.2))
...
with np.load('trajectory%d.npz' % dataset) as data:
    tract_xy = data['xy']
    tract_c = data['cell']
f = open('map%d.data' % dataset, 'rb'); map_cm = pickle.load(f); f.close()
...
plt.imshow(tex_map)
plt.title("Texture for dataset=%d" % dataset)
plt.savefig('texture of dataset= %d' % dataset)